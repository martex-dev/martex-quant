...
`martex_quant/_bundle/` at build time (secrets excluded), and this module
resolves whichever copy exists:

1. `martex_quant/_bundle/` — present in a built wheel or sdist install.
2. the repo root above `src/martex_quant/` — present in an editable/dev
...
def bundle_root() -> Path | None:
    ...
    """
    # src/martex_quant/bundle.py -> src/martex_quant -> src -> repo root
