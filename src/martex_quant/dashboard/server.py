"""Dashboard HTTP server.
    ...
    python -m martex_quant.dashboard
...
from martex_quant.dashboard.data import gather_status
from martex_quant.dashboard.lab import gather_lab
...
ACTIONS: dict[str, list[str]] = {
    "paper-run": ["{py}", "-m", "martex_quant.live.paper", "--strategy", "vol-target"],
    "mt5-dryrun": ["{py}", "-m", "martex_quant.live.trade", "--strategy", "vol-target"],
    "guard-check": ["{py}", "-m", "martex_quant.live.guard"],
    "data-report": ["{py}", "-m", "martex_quant.data.report"],
